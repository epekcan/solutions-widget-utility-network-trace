...
export class GeometryHandler {
  ...
  async getPercentageAlong(sourceGeom, flagGeom, inSR) {
  ...
  async createPolyline(geom, inSR) {
    //return new Promise(async(resolve, reject) => {
    const [{ default: Polyline }] = await Promise.all([
  ...
  async intersectToPoint(geometry, flagGeom, sourceSR) {
    let flagPointArray = [];
    const sourceLine = await this.createPolyline(geometry.paths, sourceSR.wkid);
    const flagToProjected = await this.projectGeometry(flagGeom, sourceSR);
        ...
        cg.paths.map((p) => {
          ...
          if (doesFirstIntersect) {
            if (flagPointArray.length > 0) {
              const found = flagPointArray.indexOf((fp) => {
  ...
  createBuffer(geom, distance, units) {
    return GeometryEngine.buffer(geom, distance, units);
  }
  async projectGeometry(sourceGeom, targetSR) {
    if (!projection.isLoaded()) {
      await projection.load();
    }
