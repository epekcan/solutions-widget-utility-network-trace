...
import { i as inRange, s as sameDate, b as dateFromRange, l as localizeNumber, g as getOrder, n as nextMonth, c as prevMonth, e as parseNumber } from './date-3d6f3569.js';
  ...
  render() {
    ...
    const days = [
      ...prevMonDays.map((day) => {
        ...
        return (h("calcite-date-day", { day: day, disabled: !inRange(date, this.min, this.max), localeData: this.localeData, onCalciteDaySelect: () => this.calciteDateSelect.emit(date), scale: this.scale, selected: sameDate(date, this.selectedDate) }));
      }),
      ...curMonDays.map((day) => {
        const date = new Date(year, month, day);
        const active = sameDate(date, this.activeDate);
        return (h("calcite-date-day", { active: active, "current-month": true, day: day, disabled: !inRange(date, this.min, this.max), localeData: this.localeData, onCalciteDaySelect: () => this.calciteDateSelect.emit(date), ref: (el) => {
            // when moving via keyboard, focus must be updated on active date
            if (active && this.activeFocus) {
              el === null || el === void 0 ? void 0 : el.focus();
            }
          }, scale: this.scale, selected: sameDate(date, this.selectedDate) }));
      }),
      ...nextMonDays.map((day) => {
        const date = new Date(year, month + 1, day);
        return (h("calcite-date-day", { day: day, disabled: !inRange(date, this.min, this.max), localeData: this.localeData, onCalciteDaySelect: () => this.calciteDateSelect.emit(date), scale: this.scale, selected: sameDate(date, this.selectedDate) }));
