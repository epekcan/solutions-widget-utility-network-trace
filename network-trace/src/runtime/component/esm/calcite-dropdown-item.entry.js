...
const CalciteDropdownItem = class {
  ...
  render() {
    ...
    return (h(Host, { "aria-checked": this.active.toString(), dir: dir, isLink: this.href, role: "menuitem", scale: scale, "selection-mode": this.selectionMode, tabindex: "0" }, this.selectionMode === "multi" ? (h("calcite-icon", { class: "dropdown-item-check-icon", icon: "check", scale: "s" })) : null, contentEl));
  }
  //--------------------------------------------------------------------------
  //
  //  Event Listeners
