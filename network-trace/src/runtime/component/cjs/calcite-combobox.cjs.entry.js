...
const CalciteCombobox = class {
  constructor(hostRef) {
    ...
    this.filterItems = lodashEs.debounce((value) => {
  ...
  render() {
    ...
    return (index.h(index.Host, { active: active, dir: dir }, index.h("div", { class: "selections" }, selectedItems.map((item) => {
      return (index.h("calcite-chip", { dir: dir, dismissible: true, key: item.value, scale: scale, value: item.value }, item.textLabel));
    })), index.h("div", { "aria-expanded": active.toString(), "aria-haspopup": "listbox", "aria-owns": listBoxId, ref: this.setReferenceEl, role: "combobox" }, index.h("input", { "aria-autocomplete": "list", "aria-controls": listBoxId, "aria-label": label, disabled: disabled, onBlur: this.comboboxBlurHandler, onFocus: this.comboboxFocusHandler, onInput: this.inputHandler, onKeyDown: this.handleInputKeyDown, placeholder: placeholder, ref: (el) => (this.textInput = el), type: "text" })), index.h("div", { "aria-hidden": (!active).toString(), class: "list-container", ref: this.setMenuEl }, index.h("ul", { "aria-label": label, "aria-multiselectable": "true", class: {
        list: true,
        [popper.CSS.animation]: true,
        [popper.CSS.animationActive]: active
      }, id: listBoxId, role: "listbox" }, index.h("slot", null)))));
  }
