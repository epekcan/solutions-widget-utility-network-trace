...
const CalciteDate = class {
  constructor(hostRef) {
    ...
    this.syncThisToProxyInput = () => {
      ...
      this.value = date.dateToISO(date.dateFromRange(date$1, min, max));
    };
    /**
     * Update input proxy
     */
  ...
  render() {
    ...
    return (index.h(index.Host, { dir: dir, role: "application" }, index.h("div", { class: "slot" }, index.h("slot", null)), !this.noCalendarInput && this.localeData && (index.h("div", { role: "application" }, index.h("calcite-input", { class: "input", icon: "calendar", "number-button-type": "none", onCalciteInputBlur: (e) => this.blur(e.detail), onCalciteInputFocus: () => (this.active = true), onCalciteInputInput: (e) => this.input(e.detail.value), placeholder: this.localeData.placeholder, scale: this.scale, type: "text", value: formattedDate }))), this.localeData && (index.h("div", { class: "calendar-picker-wrapper" }, index.h("calcite-date-month-header", { activeDate: activeDate, dir: dir, intlNextMonth: this.intlNextMonth, intlPrevMonth: this.intlPrevMonth, localeData: this.localeData, max: max, min: min, onCalciteActiveDateChange: (e) => {
        this.setValue(new Date(e.detail));
        this.activeDate = new Date(e.detail);
        this.calciteDateChange.emit(new Date(e.detail));
      }, scale: this.scale, selectedDate: date$1 || new Date() }), index.h("calcite-date-month", { activeDate: activeDate, dir: dir, localeData: this.localeData, max: max, min: min, onCalciteActiveDateChange: (e) => {
        this.activeDate = new Date(e.detail);
        ...
        this.calciteDateChange.emit(new Date(e.detail));
        this.reset();
      }, scale: this.scale, selectedDate: date$1 })))));
  }
  //--------------------------------------------------------------------------
  //
  //  Private Methods
  ...
  setValueAsDate(value) {
    if (value) {
      const date$1 = date.dateFromISO(value);
      if (date$1) {
