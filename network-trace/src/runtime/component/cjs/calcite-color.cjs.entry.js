...
const CalciteColor = class {
  constructor(hostRef) {
    ...
    this.handleHexInputChange = (event) => {
      ...
      if (hex !== utils.normalizeHex(color.hex())) {
        this.internalColorSet(Color__default['default'](hex));
      }
    };
    this.handleSavedColorSelect = (event) => {
      const swatch = event.currentTarget;
      this.internalColorSet(Color__default['default'](swatch.color));
    };
    ...
    this.renderChannel = (value, index$1, label, ariaLabel) => (index.h("calcite-input", { "aria-label": ariaLabel, class: resources.CSS.channel, "data-channel-index": index$1, numberButtonType: "none", onChange: this.handleChannelChange, onInput: this.handleChannelInput, prefixText: label, scale: "s", type: "number", value: value.toString() }));
    this.deleteColor = () => {
    ...
    this.drawColorFieldAndSlider = lodashEs.throttle(() => {
  ...
  handleValueChange(value, oldValue) {
    ...
    const color = Color__default['default'](value);
  ...
  render() {
    ...
    return (index.h("div", { class: resources.CSS.container }, index.h("canvas", { class: {
      ...
      } }, intlHex), index.h("calcite-color-hex-input", { class: resources.CSS.control, dir: elementDir, onCalciteColorHexInputChange: this.handleHexInputChange, ref: (node) => (this.hexInputNode = node), scale: hexInputScale, theme: theme, value: selectedColorInHex }))), hideChannels ? null : (index.h("calcite-tabs", { class: {
        [resources.CSS.colorModeContainer]: true,
        [resources.CSS.splitSection]: true
      }, dir: elementDir }, index.h("calcite-tab-nav", { slot: "tab-nav" }, this.renderChannelsTabTitle("rgb"), this.renderChannelsTabTitle("hsv")), this.renderChannelsTab("rgb"), this.renderChannelsTab("hsv"))))), hideSaved ? null : (index.h("div", { class: { [resources.CSS.savedColorsSection]: true, [resources.CSS.section]: true } }, index.h("div", { class: resources.CSS.header }, index.h("label", null, intlSaved), index.h("div", { class: resources.CSS.savedColorsButtons }, index.h("calcite-button", { appearance: "transparent", "aria-label": intlDeleteColor, class: resources.CSS.deleteColor, color: "dark", iconStart: "minus", onClick: this.deleteColor, scale: scale, theme: theme }), index.h("calcite-button", { appearance: "transparent", "aria-label": intlSaveColor, class: resources.CSS.saveColor, color: "dark", iconStart: "plus", onClick: this.saveColor, scale: scale, theme: theme }))), savedColors.length > 0 ? (index.h("div", { class: resources.CSS.savedColors }, [
  ...
  drawActiveColorFieldColor() {
    const color = this.color.hsv();
    const { dimensions: { colorField: { height, width }, thumb: { radius } } } = this;
    const x = color.saturationv() / (resources.HSV_LIMITS.s / width);
    const y = height - color.value() / (resources.HSV_LIMITS.v / height);
  ...
  drawHueSlider() {
    ...
    hueSliderColorStopKeywords.forEach((keyword) => {
      gradient.addColorStop(currentOffset, Color__default['default'](keyword).string());
  ...
  updateColorFromChannels(channels) {
    this.internalColorSet(Color__default['default'](channels, this.channelMode));
  }
