...
export default class Setting extends BaseWidgetSetting<AllWidgetSettingProps<IMConfig>, State>{
          ...
          <Tooltip title={this.formatMessage('layoutStyleBasic')} placement="bottom">
            <div>
              <Button onClick={() => { this.arrangementHandler('simple') }}
                ...
                <Icon width={68} height={68} icon={require('./assets/arrange_simple.png')} ></Icon>
              </Button>
          ...
          <Tooltip title={this.formatMessage('layoutStyleAdvanced')} placement="bottom">
            <div>
              <Button onClick={() => { this.arrangementHandler('management') }}
                ...
                <Icon width={68} height={68} icon={require('./assets/arrange_advance.png')} ></Icon>
              </Button>
  ...
  renderFieldOrderList = () => {
    ...
    return (
      <Table style={{ height: '100%' }}>
        <thead>
          <tr>
            <th style={{ fontSize: 'smaller' }}>Field</th>
            <th style={{ fontSize: 'smaller' }}>Order</th>
          </tr>
        </thead>
        <tbody>
          {fieldsList}
  ...
  renderConfigSlideout = () => {
    ...
    return (
      <SidePopper isOpen={this.state.showSidePanel} position="right">
            ...
            <div key={'refreshInterval'} style={{ paddingBottom: '5px', display: 'flex', flexDirection: 'row', flex: 1 }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <Tooltip title={this.formatMessage('settingsSectionsTooltipNumber')} placement="bottom">
                  <TextInput size={'sm'} style={{ width: 40 }} type="text" invalid={this.state.invalidInterval} placeholder="000" value={this.state.intervalNumber}
                    onChange={(evt: any) => {
                      ...
                      if (newVal.length <= 3) {
                        if (isNaN(newVal)) {
                          ...
                          if (newVal <= 0) {
                            ...
                            if (newVal.indexOf('.') > -1) {
                              this.setState({ intervalNumber: newVal.replace('.', '') });
                            } else {
                              const intValue = parseInt(newVal);
                              this.setState({ invalidInterval: false, intervalNumber: intValue }, () => {
                                this.updateAllowanceInterval(al.key, intValue);
                              });
                            }
                        ...
                        if (isNaN(newVal)) {
                          ...
                          const intValue = parseInt(newVal.substring(0, 3));
  ...
  renderServiceSelector = () => {
    ...
    return (
      <SettingSection className="map-selector-section" title={
        <React.Fragment>
          <div style={{display:'flex', flexDirection:'row'}}>
            <div style={{display:'flex', flex:1}}>{this.formatMessage('settingsSectionsVersionService')}</div>
        ...
        <SettingRow>
          <Alert color="warning" isOpen={this.state.showAlert} toggle={() => { this.setState({ showAlert: !this.state.showAlert }) }}>
            {this.state.alertMessage}
          </Alert>
        </SettingRow>
        <SettingRow>
          <div className="w-100">
            <div className="component-map-selector" style={{ maxHeight: 300, overflow: 'auto' }}>
